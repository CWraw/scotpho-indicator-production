...
create_agegroups <- function(dataset) {
  # Creates fiver year bands starting from 0 to 4 finishing at 90+
    dataset %>% mutate(age_grp = as.character(cut(age, breaks = c(seq(0, 90, by = 5), Inf),
                                      labels = seq(1, 19), right = FALSE)))
}
